...
if (isset($_POST['register'])) {
    ...
    if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z]).{8,}$/', $account_password)) {
        echo '<script>alert("Mật khẩu không đúng định dạng. Yêu cầu ít nhất 8 ký tự, bao gồm ít nhất một chữ hoa và một chữ thường.");</script>';
        header('Location:../../index.php?page=register&message=nonePass');
        exit;
    }
    ...
    if ($count == 0) {
        ...
        $sql_resgister = "INSERT INTO account(account_name ,account_email, account_phone, account_password, account_type) VALUE('" . $account_name . "', '" . $account_email . "', '" . $account_phone . "', '" . $account_password . "', 0)";
